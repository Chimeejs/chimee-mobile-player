...
var ChimeeMobilePlayer = function (_Chimee) {
  ...
  function ChimeeMobilePlayer(config) {
    ...
    var ua = new chimeeHelper.UAParser().result();
    var isUIAvailable = util_js.uiIsAvailable(defaultDisableUA, ua);

    console.warn('The UI plug-in can be configured by the attribute disableUA. \n      The current UA does not support the UI plug-in according to the default configuration.\n      Checkout the default configuration');
