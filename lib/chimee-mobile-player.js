 ...
 * @param {Boolean} capture 是否在捕获阶段监听
 */
function addDelegate$1(host, selector, type, handler, capture = false) {
  const el = host.$dom;
  const handlerWrap = function handlerWrap(e) {
 ...
 * @param {Boolean} capture 是否在捕获阶段监听
 */
function removeDelegate$1(host, selector, type, handler, capture = false) {
  const el = host.$dom;
  /* 尝试从缓存中读取包装后的方法 */
...
let Base = class Base {
  ...
  addAllEvent() {
    this.option.defaultEvent && _Object$keys(this.option.defaultEvent).forEach(item => {
      const key = this.option.defaultEvent[item];
      this[key] = bind(this[key], this);
      addDelegate$1(this.parent, this.option.tag, item, this[key], false, false);
    });
    this.option.event && _Object$keys(this.option.event).forEach(item => {
      const key = '__' + item;
  ...
  removeAllEvent() {
    this.option.defaultEvent && _Object$keys(this.option.defaultEvent).forEach(item => {
      const key = this.option.defaultEvent[item];
      this[key] = bind(this[key], this);
      removeDelegate$1(this.parent, this.option.tag, item, this[key], false, false);
    });
    this.option.event && _Object$keys(this.option.event).forEach(item => {
      const key = '__' + item;
      this[key] = bind(this.option.event[item], this);
...
let Play = class Play extends Base {
  constructor(parent, option) {
    super(parent);
...
let Screen = (_class$9 = class Screen extends Base {
  constructor(parent, option) {
    super(parent);
    this.state = 'small';
    this.option = deepAssign(defaultOption$1, isObject(option) ? option : {});
    this.init();
  }
...
}, _applyDecoratedDescriptor$8(_class$9.prototype, 'screenChange', [autobind], _Object$getOwnPropertyDescriptor(_class$9.prototype, 'screenChange'), _class$9.prototype), _class$9);
...
let ProgressBar = (_class$10 = class ProgressBar extends Base {
  constructor(parent, option) {
    super(parent);
    this.option = deepAssign(defaultOption$2, isObject(option) ? option : {});
...
let CurrentTime = class CurrentTime extends Base {
  constructor(parent, option) {
    super(parent);
    this.option = deepAssign(defaultOption$3, isObject(option) ? option : {});
    this.init();
  }
...
let TotalTime = class TotalTime extends Base {
  constructor(parent, option) {
    super(parent);
    this.option = deepAssign(defaultOption$4, isObject(option) ? option : {});
    this.init();
  }
...
function createChild(plugin) {
  ...
  const children = {};
  _Object$keys(childConfig).forEach(item => {
    switch (item) {
      case 'play':
        if (childConfig.play) {
...
const innerPlugins = [mobiControlbar.name, chimeeState.name];
...
let ChimeeMobilePlayer = class ChimeeMobilePlayer extends Chimee {
  constructor(config) {
